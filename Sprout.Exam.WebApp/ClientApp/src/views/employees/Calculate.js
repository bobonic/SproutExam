...
export class EmployeeCalculate extends Component {
    ...
    async calculateSalary() {
        ...
        const work = this.state.typeId === 1 ? this.state.absentDays : this.state.workedDays;

        const requestOptions = {
            method: 'POST',
            headers: !token ? {} : { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: this.state.id, days: work })
        };
        const response = await fetch('api/employees/' + this.state.id + '/calculate', requestOptions);
        const data = await response.json();
        const netIncome = new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'PHP' }).format(data);
        this.setState({ loadingCalculate: false, netIncome: netIncome });
    }
